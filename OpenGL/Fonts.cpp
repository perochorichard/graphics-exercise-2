...
void Fonts::Initialize(string _name, FT_UInt _size)
{
	...
	//initialize the livrary and font face
	...
	//set font height to 48 (0 to auto-calculated width)
...
{
	//load first 128 character glyph into character textures
...
void Fonts::RenderText(std::string _text, float _x, float _y, float _scale, glm::vec3 _color)
{
	...
	for (c = _text.begin(); c != _text.end(); c++)
	{
		...
		_x += (ch.Advance >> 6) * _scale;		//bitshift by 6 to fet value in pixels (2^6 = 64 (divide amount of 1/64th pixels by 64 to get amount of pixels))
